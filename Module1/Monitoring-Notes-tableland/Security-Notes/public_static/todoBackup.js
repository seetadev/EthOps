...
window.onload = function() {
    ...
    function deletedList() {
        ...
        for (var i = 0; i < deletedTasks.length; i++) {
            cont += '<div>' + '<span>'+ deletedTasks[i].task + '</span>'+ '</div>'
        }
    ...
    function refreshList() {
        ...
        for (var i = 0; i < todoList.length; i++) {
            ...
            '<input ' +
                 ...
                 ' onchange="setDone(this)" '+  ' id=' + i  + ' type="checkbox"/>' +
                   '<label for="items"'+ '>' + todoList[i].task + '</label>' +
                   '<i class="fa fa-times fa-2x" aria-hidden="true" onclick="deletefn(document.getElementById('+i+'))"></i>' + '</br>'
              + '</div>';
            /*
            DOUBT POOCH LENA
